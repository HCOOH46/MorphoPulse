...
def _downstream_meta_csv(*parts):
    return DATA_ROOT.joinpath("downstream", *parts)
...
class PPGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            signal = joblib.load(signal_segment)['ppg']
        except KeyError:
            signal = joblib.load(signal_segment)['signal']
...
class PPGDatasetLabelsArray(Dataset):
    ...
    def load_signal(self, idx):
        try:
            signal = joblib.load(self.filenames[idx])['ppg']
        except KeyError:
            signal = joblib.load(self.filenames[idx])['signal']
        if self.normalization:
            norm = Normalize(method='z-score')
    ...
    # def __getitem__(self, idx):
        ...
        # signal = torch.Tensor(self.transform(signal))
        # if self.DEBUG:
        #     print(f"Signal shape before transform: {signal.shape}")
        # return signal.squeeze(dim=1), self.labels[idx]
...
def load_dataset_obj(dataset_name, CustomDataset, label_name, fs_target, normalization, simclr, transform):
    ...
    if dataset_name == "mesa":
        case_name = "mesaid"
        path = _pretrain_signal_dir("MESA", "subject")
        fs=256
        usecols = [case_name, "segments", "age", label_name]

        df_train = pd.read_csv(_downstream_meta_csv("mesa", "train_clean.csv"), usecols=usecols)
        df_val = pd.read_csv(_downstream_meta_csv("mesa", "val_clean.csv"), usecols=usecols)
        df_test = pd.read_csv(_downstream_meta_csv("mesa", "test_clean.csv"), usecols=usecols)
    ...
    if dataset_name == "vital":
        path = _pretrain_signal_dir("PulseDB_Vital", "subject")
        case_name = "caseid"
        fs=500
        usecols = [case_name, "segments", "age"]

        df_train = pd.read_csv(_downstream_meta_csv("vital", "train_clean.csv"), usecols=usecols)
        df_val = pd.read_csv(_downstream_meta_csv("vital", "val_clean.csv"), usecols=usecols)
        df_test = pd.read_csv(_downstream_meta_csv("vital", "test_clean.csv"), usecols=usecols)
    ...
    if dataset_name == "mimic":
        ...
        path = _pretrain_signal_dir("PulseDB_MIMIC", "subject")
        fs=125
        usecols = [case_name, "segments", "age"]

        df_train = pd.read_csv(_downstream_meta_csv("mimic", "train_clean.csv"), usecols=usecols)
        df_val = pd.read_csv(_downstream_meta_csv("mimic", "val_clean.csv"), usecols=usecols)
        df_test = pd.read_csv(_downstream_meta_csv("mimic", "test_clean.csv"), usecols=usecols)
