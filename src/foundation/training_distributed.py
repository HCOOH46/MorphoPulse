...
def ddp_setup(rank, world_size):
    ...
    if "MASTER_PORT" not in os.environ:
        os.environ["MASTER_PORT"] = str(random.randint(12000, 65535))
...
def save_model(model, directory, filename, content, prefix=""):
    ...
    target_dir = ensure_dir(FOUNDATION_CHECKPOINT_ROOT / directory)
    torch.save(model.state_dict(), target_dir / f"{filename}_{content}.pt")
