...
from project_paths import GENERATION_CHECKPOINT_ROOT, ensure_dir
...
def load_args():
    ...
    parser.add_argument('--feature_extractor_type', type=str, default='papagei', help='papagei | rocket | rocket for evaluation.')
    parser.add_argument('--use_custom_dataset', type=str2bool, default=False, help='Using a custom dataset, then set it to True.')
    parser.add_argument('--DDiT', type=str2bool, default=False, help='Using DDiT.')
...
def train_stage2(config: dict,
    ...
    train_exp = ExpStage2(dataset_name, in_channels, input_length, config, n_classes, feature_extractor_type, use_custom_dataset, diffusion=diffusion)
    ...
    ensure_dir(GENERATION_CHECKPOINT_ROOT)
    if diffusion:
        save_file = f'stage2-{dataset_name}-DDiT.ckpt'
    else:
        save_file = f'stage2-{dataset_name}.ckpt'
    trainer.save_checkpoint(os.path.join(GENERATION_CHECKPOINT_ROOT, save_file))
...
if __name__ == '__main__':
    ...
    for dataset_name in dataset_names:
        ...
        if not args.use_custom_dataset:
            ...
            dataset_importer = DatasetImporterCustom(dataset_name, **config['dataset'])
        ...
        # train
        train_stage2(config, dataset_name, train_data_loader, test_data_loader, args.gpu_device_ind, args.feature_extractor_type, args.use_custom_dataset, args.DDiT)
