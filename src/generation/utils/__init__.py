...
import tarfile
import zipfile
from project_paths import PROJECT_ROOT, GENERATION_CHECKPOINT_ROOT
...
def save_model(models_dict: dict, dirname='saved_models', id: str = ''):
    ...
    """
    try:
        save_root = GENERATION_CHECKPOINT_ROOT if dirname == 'saved_models' else get_root_dir().joinpath(dirname)
        ...
        for model_name, model in models_dict.items():
            torch.save(model.state_dict(), Path(save_root).joinpath(model_name + id_ + '.ckpt'))
        ...
        if id != '':
            id_ = '-' + id_
        for model_name, model in models_dict.items():
            torch.save(model.state_dict(), Path(dirname).joinpath(model_name + id_ + '.ckpt'))
